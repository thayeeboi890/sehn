...
int theme_apply(AppState* state, const char* theme_name)
{
    ...
    if (ui) {
        ...
        if (!bg.empty())
            current_theme.background = theme_parse_color(bg.c_str());
        std::string fg = read_str(ui, "foreground", "");
        if (!fg.empty())
            current_theme.foreground = theme_parse_color(fg.c_str());

        std::string panel_bg = read_str(ui, "panel_bg", "");
        if (!panel_bg.empty())
            current_theme.panel_bg = theme_parse_color(panel_bg.c_str());
        std::string panel_sep = read_str(ui, "panel_separator", "");
        if (!panel_sep.empty())
            current_theme.panel_separator = theme_parse_color(panel_sep.c_str());
        std::string btn_bg = read_str(ui, "button_bg", "");
        if (!btn_bg.empty())
            current_theme.button_bg = theme_parse_color(btn_bg.c_str());
        std::string btn_border = read_str(ui, "button_border", "");
        if (!btn_border.empty())
            current_theme.button_border = theme_parse_color(btn_border.c_str());
        std::string btn_text = read_str(ui, "button_text", "");
        if (!btn_text.empty())
            current_theme.button_text = theme_parse_color(btn_text.c_str());

        std::string overlay = read_str(ui, "overlay_text", "");
        if (!overlay.empty())
            current_theme.overlay_text = theme_parse_color(overlay.c_str());
        std::string rec = read_str(ui, "rec_color", "");
        if (!rec.empty())
            current_theme.rec_color = theme_parse_color(rec.c_str());

        // optional numeric values
        toml_datum_t dpad = toml_int_in(ui, "panel_padding");
        if (dpad.ok)
            current_theme.panel_padding = (int)dpad.u.i;
        toml_datum_t dbtn = toml_int_in(ui, "panel_button_size");
        if (dbtn.ok)
            current_theme.panel_button_size = (int)dbtn.u.i;
        toml_datum_t ofsz = toml_int_in(ui, "overlay_font_size");
        if (ofsz.ok)
            current_theme.overlay_font_size = (int)ofsz.u.i;
        toml_datum_t om = toml_int_in(ui, "overlay_margin");
        if (om.ok)
            current_theme.overlay_margin = (int)om.u.i;
        toml_datum_t dpi = toml_int_in(ui, "dpi_scale");
        if (dpi.ok)
            current_theme.dpi_scale = (float)dpi.u.i;
    ...
    // variants: if user requested a variant like "dark.compact", caller handles splitting and will
    // call apply with base then variant table
